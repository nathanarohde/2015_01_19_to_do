...
describe(List) do
  ...
  describe("#==") do
    it("is the same list if it hs the same name") do
  ...
  describe("#tasks")do
    it("returns an array of tasks for that list") do
      test_list = List.new({:name => "Epicodus stuff", :id => nil})
      test_list.save()
      test_task = Task.new({:description => "Learn SQL", :due_date => 20150522, :list_id => test_list.id()})
      test_task.save()
      test_task2 = Task.new({:description => "Review Ruby", :due_date => 20150522, :list_id => test_list.id()})
      test_task2.save()
